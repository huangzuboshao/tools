...
public class RsaEncryptUtils {
    ...
    public static String encryptByPublicKey(String data) throws Exception {
        RSAPublicKey publicKey = RsaEncryptUtils.getPublicKey(modulus, public_exponent);
        Cipher cipher = Cipher.getInstance("RSA");
        cipher.init(Cipher.ENCRYPT_MODE, publicKey);
        // 模长
        int keyLength = publicKey.getModulus().bitLength() / 8;
        // 加密数据长度 <= 模长-11
        String[] dataArr = splitString(data, keyLength - 11);
    ...
    public static String decryptByPrivateKey(String data) throws Exception {
        RSAPrivateKey privateKey = RsaEncryptUtils.getPrivateKey(modulus, private_exponent);
        Cipher cipher = Cipher.getInstance("RSA");
    ...
    public static String bcd2Str(byte[] bytes) {
        char[] tempArr = new char[bytes.length * 2];
        char val;

        for (int i = 0; i < bytes.length; i++) {
            val = (char) (((bytes[i] & 0xf0) >> 4) & 0x0f);
            tempArr[i * 2] = (char) (val > 9 ? val + 'A' - 10 : val + '0');

            val = (char) (bytes[i] & 0x0f);
            tempArr[i * 2 + 1] = (char) (val > 9 ? val + 'A' - 10 : val + '0');
        }
    ...
    /*public static void main(String[] args) throws Exception {
        ...
        String ciphertext = encryptByPublicKey("yinjihuan");
